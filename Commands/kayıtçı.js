...
module.exports.run = async(client, message, args, embed) => {
    ...
    if (member.roles.cache.has(ayar.roles.ilkyetki), await member.roles.cache.has(ayar.roles.registerStaff)) {
        await member.roles.remove(ayar.roles.ilkyetki), await member.roles.remove(ayar.roles.registerStaff).catch(err => {});
        message.channel.send(embed.setDescription(`${member}, üyesinden ${message.guild.roles.cache.get(ayar.roles.ilkyetki)} rolü alındı.`)).sil(7)

    } else {

        await member.roles.add(ayar.roles.ilkyetki), await member.roles.add(ayar.roles.registerStaff).catch(err => {});
        message.channel.send(embed.setDescription(`${member}, üyesine ${message.guild.roles.cache.get(ayar.roles.ilkyetki)} rolü verildi.`)).sil(7)
    }
