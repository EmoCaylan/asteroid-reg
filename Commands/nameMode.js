...
if(!data) new registerData({guildID: message.guild.id, tagMode: false}).save();
    switch (args[0]) {
        case "aç":
            if (data.nameMode === true) return message.channel.send(embed.setDescription(`${client.emojis.cache.get(ayar.emojis.no)} İsim modu zaten aktif`));
            data.nameMode = true;
            data.save();
            message.channel.send(embed.setDescription(`${message.member}, İsim modu başarıyla açıldı!`)).sil(7);
            break;
        case "kapat":
            if (data.nameMode === false) return message.channel.send(embed.setDescription(`${client.emojis.cache.get(ayar.emojis.no)} İsim modu zaten deaktif`));
            data.nameMode = false;
            data.save();
