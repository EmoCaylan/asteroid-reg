...
module.exports.run = async(client, message, args, embed) => {
    ...
    if (data && data.tagMode === true) {
        if (ayar.guild.tagges.some(s => !member.user.tag.includes(s.toUpperCase())) && !member.roles.cache.has(ayar.roles.vipRole) && !member.roles.cache.has(ayar.roles.boosterRole)) return message.channel.send(embed.setDescription(`${member}, Adlı kullanıcıda tag bulunmadığı için işlem gerçekleştirilemedi.`)).sil(7);
    }
