...
let embed = new MessageEmbed().setColor('RANDOM').setTimestamp().setFooter('✫ A S T E R O I D')
...
class Register {
    ...
    static async man(user, admin, name, channel) {
        ...
        if (nameMode && nameMode.nameMode === true) { channel.send(embed.setDescription(`${user} üyesinin ismi başarıyla \`${name}\` olarak değiştirildi. Bu üye daha önce bu isimlerle kayıt olmuş. \n\n ${user.guild.emojis.cache.get(ayar.emojis.yes)} Kişinini toplamda **${data.length}** isim kayıtı bulundu. \n ${isimler} \n\n Kişinin önceki isimlerine \`.isimler @Emirhan/ID\` komutuyla bakarak kayıt işlemini gerçekleştirmeniz önerilir.`)).sil(10); } else { channel.send(embed.setDescription(`${user} üyesine ${user.guild.roles.cache.get(ayar.roles.manRoles[0])} rolü verildi.`)).sil(7); }
        if (ayar.guild.tagges.some(s => user.user.tag.includes(s.toUpperCase()))) user.roles.add(ayar.roles.tagRole)
    }
    ...
    static async woman(user, admin, name, channel) {
        ...
        if (nameMode && nameMode.nameMode === true) { channel.send(embed.setDescription(`${user} üyesinin ismi başarıyla \`${name}\` olarak değiştirildi. Bu üye daha önce bu isimlerle kayıt olmuş. \n\n ${user.guild.emojis.cache.get(ayar.emojis.yes)} Kişinini toplamda **${data.length}** isim kayıtı bulundu. \n ${isimler} \n\n Kişinin önceki isimlerine \`.isimler @Emirhan/ID\` komutuyla bakarak kayıt işlemini gerçekleştirmeniz önerilir.`)).sil(10); } else { channel.send(embed.setDescription(`${user} üyesine ${user.guild.roles.cache.get(ayar.roles.womanRoles[0])} rolü verildi.`)).sil(7); }
        if (ayar.guild.tagges.some(s => user.user.tag.includes(s.toUpperCase()))) user.roles.add(ayar.roles.tagRole)
    }

    static fixname(member, isim, yaş) {
        let fixTag = `${ayar.guild.tagges.some(s => member.user.tag.includes(s.toUpperCase())) ? ayar.guild.tag : ayar.guild.defaultTag}`
        var name;
