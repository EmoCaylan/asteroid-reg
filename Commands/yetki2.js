...
module.exports.run = async(client, message, args, embed) => {
    ...
    if (member.roles.cache.has(ayar.roles.yetki2), await member.roles.cache.has(ayar.roles.mute)) {
        await member.roles.remove(ayar.roles.yetki2), await member.roles.remove(ayar.roles.mute).catch(err => {});
        message.channel.send(embed.setDescription(`${member}, üyesinden ${message.guild.roles.cache.get(ayar.roles.yetki2)} rolü alındı.`)).sil(7)

    } else {

        await member.roles.add(ayar.roles.yetki2), await member.roles.add(ayar.roles.mute).catch(err => {});
        message.channel.send(embed.setDescription(`${member}, üyesine ${message.guild.roles.cache.get(ayar.roles.yetki2)} rolü verildi.`)).sil(7)
    }
